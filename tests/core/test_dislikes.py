...
class DislikeStoreTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        self.store = DislikeStore(self.root, clock=lambda: NOW)
    ...
    def test_initializes_empty_registry(self) -> None:
        ...
        self.assertEqual(data, {"schema_version": 1, "dislikes": {}})
    ...
    def test_add_persists_metadata_relationship(self) -> None:
        ...
        self.assertEqual(self.store.get(record["dislike_id"]), record)
    ...
    def test_duplicate_relationship_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(DislikeError, "already exists"):
            self.store.add(USER_A, "jellyfin", "item-1", media_type="tv")
    ...
    def test_remove_updates_registry_and_deletes_record(self) -> None:
        ...
        path = self.store._record_file(record["dislike_id"])
        removed = self.store.remove(record["dislike_id"])
    ...
    def test_validation_rejects_bad_values_and_large_metadata(self) -> None:
        with self.assertRaises(DislikeError):
    ...
    def test_verify_detects_registry_mismatch_orphan_and_escape(self) -> None:
        ...
        registry["dislikes"][record["dislike_id"]]["item_id"] = "wrong"
        self.store.registry_file.write_text(json.dumps(registry))
        orphan = self.store.records_directory / ("dis_" + "f" * 32 + ".json")
        orphan.write_text("{}")
        errors = self.store.verify()
        self.assertTrue(any("item_id does not match" in error for error in errors))
        self.assertTrue(any("unregistered dislike record" in error for error in errors))

        registry["dislikes"][record["dislike_id"]]["path"] = "../../escape.json"
        self.store.registry_file.write_text(json.dumps(registry))
        self.assertTrue(any("escapes dislikes directory" in error for error in self.store.verify()))

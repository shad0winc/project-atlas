...
from atlas.dislike_service import DislikeService
...
class DislikeServiceTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(); self.store = DislikeStore(Path(self.temp.name)); self.events=[]
    ...
    def test_add_enriches_record_and_publishes_event(self):
        service=DislikeService(self.store,{"jellyfin":StubProvider()},self.publisher)
        ...
        self.assertEqual("dislike.created",self.events[0][0]); self.assertIsNone(result.event_error)
    def test_remove_publishes_event(self):
        service=DislikeService(self.store,{"jellyfin":StubProvider()},self.publisher)
        record=service.add("usr_"+"b"*32,"jellyfin","abc").record; self.events.clear()
        result=service.remove(record["dislike_id"])
        self.assertEqual("dislike.removed",self.events[0][0]); self.assertEqual(record["dislike_id"],result.record["dislike_id"])
    def test_provider_failure_and_unknown_provider_are_errors(self):
        service=DislikeService(self.store,{"jellyfin":StubProvider(MediaProviderError("missing"))})
        with self.assertRaisesRegex(DislikeError,"missing"): service.add("usr_"+"c"*32,"jellyfin","abc")
        with self.assertRaisesRegex(DislikeError,"unsupported"): service.add("usr_"+"c"*32,"plex","abc")
    def test_event_failure_does_not_rollback(self):
        def fail(*_): raise RuntimeError("event down")
        service=DislikeService(self.store,{"jellyfin":StubProvider()},fail)

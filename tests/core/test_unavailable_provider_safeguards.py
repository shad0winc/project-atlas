...
def test_healthy_operations_preserve_subscribed_terminal_game_when_provider_omits_it(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A healthy provider omission must not erase subscribed terminal memory."""
    _, _, worker = _sports_modules(monkeypatch)

...
def _add_profile_options(
    ...
    *,
    ...
    role_group.add_argument(
        "--role",
        choices=tuple(
            sorted(LEGACY_ROLES | VALID_ROLES)
        ),
        help=(
            "Assign one role. Legacy admin and user aliases "
            "remain supported."
        ),
    )
    ...
    role_group.add_argument(
        "--roles",
        nargs="+",
        choices=tuple(sorted(VALID_ROLES)),
        help="Assign one or more Atlas roles.",
    )
    ...
    parser.add_argument(
        ...
        metavar="PERMISSION",
        help=(
            "Add a direct permission grant. Repeat for "
    ...
    parser.add_argument(
        ...
        metavar="PERMISSION",
        help=(
            "Add a direct permission denial. Repeat for "
            "multiple permissions."
        ),
    )
...
def _profile_values(
    ...
    *,
    ...
    values = _provided(args, names)

    if args.grant is not None or args.deny is not None:
        values["permission_overrides"] = {
            "allow": args.grant or [],
            "deny": args.deny or [],
        }
...
def main(
    ...
    try:
        ...
        if args.action == "show":
            print(
                json.dumps(
                    store.get_user(args.identifier),
                    indent=2,
                    sort_keys=True,
                )
            )
        ...
        if args.action == "link-jellyfin":
            try:
                jellyfin_user = (
                    ...
                    .get_user(args.jellyfin_user_id)
                )
            except MediaProviderError as error:
                raise UserProfileError(
                    f"unable to link Jellyfin user: {error}"
                ) from error

...
atlas_backup_recovery_surface_rows() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    'dislikes' \
    "$ATLAS_IDENTITY_DIR/dislikes" \
    'state/identity/dislikes' \
    'required' \
    'directory' \
    'identity'
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$sports_root/state/dispatcharr-channel-bindings.json" \
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    'sports-live-sources' \
    "$sports_root/state/live-sources.json" \
...
atlas_backup_recovery_validate_registry() {
  ...
  [[ "$count" -eq 16 ]] || {
    printf 'ERROR: expected 16 recovery surfaces, found %s.\n' "$count" >&2
    return 1
  }
}
...
atlas_backup_recovery_validate_archive() {
  ...
  [[ "$project_rows" -eq 1 && "$row_count" -eq 17 ]] || {
...
atlas_backup_recovery_restore_plan() {
  ...
  [[ "$row_count" -eq 16 ]] || {
    printf 'ERROR: restore plan expected 16 surfaces, found %s.\n' \
      "$row_count" >&2
    return 1
  }
}
...
if len(rows) != 16:
    raise SystemExit("ERROR: restore apply plan must contain 16 surfaces")
